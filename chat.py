...
try:
    def main():
        ...
        if len(sys.argv) > 1:
            ...
            #Creating cookies so every time we login, we do not have to scan the QR from whatsapp web.
            ...
            while True:
                ...
                if isConnected.lower() == 'y':
                    break
            # Checking if the word 'WhatsApp' present in the title of the page 
    ...
    def printContacts(driver):
        contacts = driver.find_element(By.XPATH, './/div[@class="contact chat"]')
        for contact in contacts:
            phone = None
            name = None
            name_section = contact.find('.//span[@class="screen-name-text"]')
            print(name_section)
